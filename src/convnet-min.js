...
(function(c) {
	...
	var k = function() {
		...
		var s = Math.sqrt(-2 * Math.log(q) / q);
		d = o * s;
		j = true;
		return p * s
	};
	var h = function(p, o) {
		return Math.random() * (o - p) + p
	};
	var f = function(p, o) {
		return Math.floor(Math.random() * (o - p) + p)
	};
	var b = function(p, o) {
		return p + k() * o
	};
	...
	var l = function(p, q, o) {
		return typeof p[q] !== "undefined" ? p[q] : o
	};
	c.randf = h;
	c.randi = f;
	c.randn = b;
...
(function(c) {
	...
	var d = function(g) {
		...
		this.l1_decay_mul = typeof g.l1_decay_mul !== "undefined" ? g.l1_decay_mul : 0;
		this.l2_decay_mul = typeof g.l2_decay_mul !== "undefined" ? g.l2_decay_mul : 1;
	...
	d.prototype = {
		forward: function(l, s) {
			this.in_act = l;
			var j = new a(this.out_sx, this.out_sy, this.out_depth, 0);
			for(var o = 0; o < this.out_depth; o++) {
				var n = this.filters[o];
				var r = -this.pad;
				var p = -this.pad;
				for(var e = 0; e < this.out_sx; r += this.stride, e++) {
					p = -this.pad;
					for(var t = 0; t < this.out_sy; p += this.stride, t++) {
						var q = 0;
						for(var m = 0; m < n.sx; m++) {
							for(var k = 0; k < n.sy; k++) {
								for(var i = 0; i < n.depth; i++) {
									var g = p + k;
									var h = r + m;
									if(g >= 0 && g < l.sy && h >= 0 && h < l.sx) {
										q += n.w[((n.sx * k) + m) * n.depth + i] * l.w[((l.sx * g) + h) * l.depth + i]
									}
								}
							}
						}
		...
		backward: function() {
			var k = this.in_act;
			k.dw = c.zeros(k.w.length);
			for(var n = 0; n < this.out_depth; n++) {
				var m = this.filters[n];
				var q = -this.pad;
				var p = -this.pad;
				for(var e = 0; e < this.out_sx; q += this.stride, e++) {
					p = -this.pad;
					for(var t = 0; t < this.out_sy; p += this.stride, t++) {
						var o = this.out_act.get_grad(e, t, n);
						for(var l = 0; l < m.sx; l++) {
							for(var j = 0; j < m.sy; j++) {
								for(var i = 0; i < m.depth; i++) {
									var g = p + j;
									var h = q + l;
									if(g >= 0 && g < k.sy && h >= 0 && h < k.sx) {
										var s = ((k.sx * g) + h) * k.depth + i;
										var r = ((m.sx * j) + l) * m.depth + i;
										m.dw[r] += k.w[s] * o;
										k.dw[s] += m.w[r] * o
									}
								}
		...
		fromJSON: function(g) {
			...
			this.in_depth = g.in_depth;
			this.filters = [];
			this.l1_decay_mul = typeof g.l1_decay_mul !== "undefined" ? g.l1_decay_mul : 1;
			this.l2_decay_mul = typeof g.l2_decay_mul !== "undefined" ? g.l2_decay_mul : 1;
			this.pad = typeof g.pad !== "undefined" ? g.pad : 0;
	...
	var b = function(g) {
		var g = g || {};
		this.out_depth = typeof g.num_neurons !== "undefined" ? g.num_neurons : g.filters;
		this.l1_decay_mul = typeof g.l1_decay_mul !== "undefined" ? g.l1_decay_mul : 0;
		this.l2_decay_mul = typeof g.l2_decay_mul !== "undefined" ? g.l2_decay_mul : 1;
...
(function(c) {
	...
	var b = function(d) {
		var d = d || {};
		this.out_sx = typeof d.out_sx !== "undefined" ? d.out_sx : d.in_sx;
		this.out_sy = typeof d.out_sy !== "undefined" ? d.out_sy : d.in_sy;
		this.out_depth = typeof d.out_depth !== "undefined" ? d.out_depth : d.in_depth;
		this.layer_type = "input"
	};
	b.prototype = {
		forward: function(d, e) {
			this.in_act = d;
			this.out_act = d;
			return this.out_act
		},
			...
			for(var n = 0; n < f.sx; n++) {
				for(var m = 0; m < f.sy; m++) {
					for(var h = 0; h < f.depth; h++) {
						var l = f.get(n, m, h);
						var o = 0;
						for(var g = Math.max(0, h - k); g <= Math.min(h + k, f.depth - 1); g++) {
							var d = f.get(n, m, g);
							o += d * d
						}
						o *= this.alpha / this.n;
			...
			for(var r = 0; r < f.sx; r++) {
				for(var q = 0; q < f.sy; q++) {
					for(var l = 0; l < f.depth; l++) {
						...
						var s = e * e;
						for(var h = Math.max(0, l - n); h <= Math.min(l + n, f.depth - 1); h++) {
							var o = f.get(r, q, h);
							var m = -o * this.beta * Math.pow(k, this.beta - 1) * this.alpha / this.n * 2 * o;
							if(h === l) {
								m += e
							}
							m /= s;
...
(function(c) {
	...
	var b = function(d) {
		...
		this.out_sx = d.in_sx;
		this.out_sy = d.in_sy;
		this.out_depth = d.in_depth + d.in_depth * d.in_depth;
		this.layer_type = "quadtransform"
	};
	b.prototype = {
		forward: function(d, n) {
			this.in_act = d;
			var j = this.out_depth;
			var e = d.depth;
			var m = new a(this.out_sx, this.out_sy, this.out_depth, 0);
			for(var l = 0; l < d.sx; l++) {
				for(var k = 0; k < d.sy; k++) {
					for(var h = 0; h < j; h++) {
						if(h < e) {
							m.set(l, k, h, d.get(l, k, h))
						} else {
							var g = Math.floor((h - e) / e);
							var f = (h - e) - g * e;
							m.set(l, k, h, d.get(l, k, g) * d.get(l, k, f))
						}
					}
				}
			}
		...
		backward: function() {
			var d = this.in_act;
			d.dw = c.zeros(d.w.length);
			var n = this.out_act;
			var j = this.out_depth;
			var e = d.depth;
			for(var m = 0; m < d.sx; m++) {
				for(var l = 0; l < d.sy; l++) {
					for(var h = 0; h < j; h++) {
						var k = n.get_grad(m, l, h);
						if(h < e) {
							d.add_grad(m, l, h, k)
						} else {
							var g = Math.floor((h - e) / e);
							var f = (h - e) - g * e;
							d.add_grad(m, l, g, d.get(m, l, f) * k);
							d.add_grad(m, l, f, d.get(m, l, g) * k)
						}
					}
				}
			}
...
(function(c) {
	...
	b.prototype = {
		makeLayers: function(d) {
			...
			var e = function() {
				...
				for(var k = 0; k < d.length; k++) {
					var m = d[k];
					if(m.type === "softmax" || m.type === "svm") {
						l.push({
							type: "fc",
							num_neurons: m.num_classes
						})
					}
					...
					if((m.type === "fc" || m.type === "conv") && typeof(m.bias_pref) === "undefined") {
						m.bias_pref = 0;
						if(typeof m.activation !== "undefined" && m.activation === "relu") {
							m.bias_pref = 0.1
						}
					}
					if(typeof m.tensor !== "undefined") {
						if(m.tensor) {
							l.push({
								type: "quadtransform"
							})
					...
					l.push(m);
					if(typeof m.activation !== "undefined") {
						if(m.activation === "relu") {
							...
							if(m.activation === "sigmoid") {
								...
								if(m.activation === "tanh") {
									l.push({
										type: "tanh"
									})
								} else {
									if(m.activation === "maxout") {
										var j = m.group_size !== "undefined" ? m.group_size : 2;
										l.push({
											type: "maxout",
											group_size: j
										})
									} else {
										console.log("ERROR unsupported activation " + m.activation)
									}
								}
							}
						}
					}
					if(typeof m.drop_prob !== "undefined" && m.type !== "dropout") {
						l.push({
							type: "dropout",
			...
			for(var f = 0; f < d.length; f++) {
				var h = d[f];
				if(f > 0) {
					var g = this.layers[f - 1];
					h.in_sx = g.out_sx;
					h.in_sy = g.out_sy;
					h.in_depth = g.out_depth
				}
				switch(h.type) {
					case "fc":
						this.layers.push(new c.FullyConnLayer(h));
						break;
					case "lrn":
						this.layers.push(new c.LocalResponseNormalizationLayer(h));
						break;
					case "dropout":
						this.layers.push(new c.DropoutLayer(h));
						break;
					case "input":
						this.layers.push(new c.InputLayer(h));
						break;
					case "softmax":
						this.layers.push(new c.SoftmaxLayer(h));
						break;
					case "regression":
						this.layers.push(new c.RegressionLayer(h));
						break;
					case "conv":
						this.layers.push(new c.ConvLayer(h));
						break;
					case "pool":
						this.layers.push(new c.PoolLayer(h));
						break;
					case "relu":
						this.layers.push(new c.ReluLayer(h));
						break;
					case "sigmoid":
						this.layers.push(new c.SigmoidLayer(h));
						break;
					case "tanh":
						this.layers.push(new c.TanhLayer(h));
						break;
					case "maxout":
						this.layers.push(new c.MaxoutLayer(h));
						break;
					case "quadtransform":
						this.layers.push(new c.QuadTransformLayer(h));
						break;
					case "svm":
						this.layers.push(new c.SVMLayer(h));
						break;
					default:
		...
		forward: function(e, g) {
			...
			var d = this.layers[0].forward(e, g);
			for(var f = 1; f < this.layers.length; f++) {
				d = this.layers[f].forward(d, g)
			}
			return d
		},
		getCostLoss: function(d, g) {
			this.forward(d, false);
			var f = this.layers.length;
			var e = this.layers[f - 1].backward(g);
			return e
		},
		backward: function(g) {
			var f = this.layers.length;
		...
		getParamsAndGrads: function() {
			...
			for(var f = 0; f < this.layers.length; f++) {
				var g = this.layers[f].getParamsAndGrads();
		...
		getPrediction: function() {
			var g = this.layers[this.layers.length - 1];
			var h = g.out_act.w;
			var d = h[0];
			var e = 0;
		...
		toJSON: function() {
			...
			for(var d = 0; d < this.layers.length; d++) {
				e.layers.push(this.layers[d].toJSON())
			}
			return e
		},
		fromJSON: function(h) {
			this.layers = [];
			for(var g = 0; g < h.layers.length; g++) {
				var e = h.layers[g];
				var f = e.layer_type;
				var d;
				if(f === "input") {
				...
				if(f === "sigmoid") {
					d = new c.SigmoidLayer()
				}
				if(f === "tanh") {
					d = new c.TanhLayer()
				}
				if(f === "dropout") {
					d = new c.DropoutLayer()
				}
				if(f === "conv") {
					d = new c.ConvLayer()
				}
				...
				if(f === "lrn") {
					d = new c.LocalResponseNormalizationLayer()
				}
				if(f === "softmax") {
					d = new c.SoftmaxLayer()
				}
				if(f === "regression") {
					d = new c.RegressionLayer()
				}
				if(f === "fc") {
					d = new c.FullyConnLayer()
				}
				if(f === "maxout") {
					d = new c.MaxoutLayer()
				}
				if(f === "quadtransform") {
					d = new c.QuadTransformLayer()
				}
				if(f === "svm") {
					d = new c.SVMLayer()
				}
...
(function(b) {
	...
	var c = function(e, d) {
		this.net = e;
		var d = d || {};
		this.learning_rate = typeof d.learning_rate !== "undefined" ? d.learning_rate : 0.01;
...
(function(c) {
	...
	a.prototype = {
		...
		predict_soft: function(q) {
			var m = Math.min(this.ensemble_size, this.evaluated_candidates.length);
			if(m === 0) {
				return new convnetjs.Vol(0, 0, 0)
			}
			var p, t;
			for(var o = 0; o < m; o++) {
				var r = this.evaluated_candidates[o].net;
				var l = r.forward(q);
				if(o === 0) {
					p = l;
					t = l.w.length
